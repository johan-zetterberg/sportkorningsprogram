...
export function buildClassMappingHtml(uniqueXmlClasses = [], appClassList = []) {
    ...
    const classRows = uniqueXmlClasses.map((item, index) => `
      ...
      <div class="text-sm">
        <span class="font-semibold dark:text-gray-200">FrÃ¥n fil:</span>
      ...
      <div>
        ...
        <select id="mapping_${index}" data-key="${escapeAdminImportHtml(item.key)}" class="mt-1 block w-full p-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200">
          <option value="">-- VÃ¤lj klass --</option>${appClassOptions}
        </select>
      </div>
    </div>`).join('');

    return `<h3 class="font-semibold mb-2 dark:text-gray-200"> Steg 2: Mappa tÃ¤vlingsklasser</h3>
    ...
    <label class="inline-flex items-center gap-2 text-sm dark:text-amber-100">
      ...
      <span>SammanslÃ¥ per test (ignorera HÃ¤st/Ponny & Enbet/Par)</span>
    </label>
    <p class="text-xs text-amber-700 dark:text-amber-300 mt-1">NÃ¤r detta Ã¤r valt sparas Ã¤ven fÃ¤lten <code>mergedTestKey</code>/<code>mergedTestLabel</code> samt flaggan <code>useMergedTestForDisplay</code> pÃ¥ ekipagen.</p>
  </div>
        <button id="eqXmlDoFinalImport" class="mt-4 w-full bg-emerald-600 text-white py-2 px-4 rounded-lg hover:bg-emerald-700">SlutfÃ¶r Import</button>`;
}
